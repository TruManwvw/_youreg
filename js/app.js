(() => {
   ...
   (() => {
      ...
      let _slideToggle = (target, duration = 500, spollerOrientation) => {
         if ("down" == spollerOrientation) if (target.hidden) return _slideDown(target, duration); else return _slideUp(target, duration);
         if ("left" == spollerOrientation) if (target.hidden) return _slideRight(target, duration); else return _slideLeft(target, duration);
      };
         ...
         if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
               ...
               if (el.closest("[data-spoller]")) {
                  ...
                  const spollerOrientation = spollersBlock.dataset.spollersOrientation ? spollersBlock.dataset.spollersOrientation : "down";
                  if (!spollersBlock.querySelectorAll("._slide").length) {
                     if (oneSpoller && !spollerTitle.classList.contains("_spoller-active")) hideSpollersBody(spollersBlock);
                     spollerTitle.classList.toggle("_spoller-active");
                     _slideToggle(spollerTitle.nextElementSibling, spollerSpeed, spollerOrientation);
                  }
                  e.preventDefault();
               }
            }
            function hideSpollersBody(spollersBlock) {
               ...
               if (spollerActiveTitle && !spollersBlock.querySelectorAll("._slide").length) {
                  ...
                  if ("down" == spollerOrientation) _slideUp(spollerActiveTitle.nextElementSibling, spollerSpeed); else if ("left" == spollerOrientation) _slideLeft(spollerActiveTitle.nextElementSibling, spollerSpeed);
               }
            }
      ...
      function go() {
         ...
         if (screenWidth < "1170") document.querySelector(".procedure__items").removeAttribute("data-spollers-orientation"); else if (!document.querySelector(".procedure__items").hasAttribute("data-spollers-orientation")) document.querySelector(".procedure__items").setAttribute("data-spollers-orientation", "left");
      }
      ...
      function documentActions(e) {
         ...
         if (el.closest(".item-procedure")) {
            ...
            if (spollersClose.length) spollersClose.forEach((spollerClose => {
               if (!spollerClose.querySelector(".item-procedure__button").classList.contains("_spoller-active")) spollerClose.classList.remove("_active");
            }));
            if (el.classList.contains("_spoller-active")) spollerBlock.classList.add("_active");
         }
         if (el.hasAttribute("data-tabs-syncron")) {
            const titleAttribute = el.dataset.tabsSyncron;
            const titles = document.querySelectorAll("[data-tabs-syncron]");
            el.closest("[data-tabs]");
            titles.forEach((titles => {
               if (titles.classList.contains("_tab-active")) {
                  titles.classList.remove("_tab-active");
                  tabs();
               }
            }));
            const titlesSyncron = document.querySelectorAll('[data-tabs-syncron="' + titleAttribute + '"]');
            titlesSyncron.forEach((titleSyncron => {
               if (!titleSyncron.classList.contains("_tab-active")) {
                  titleSyncron.classList.add("_tab-active");
                  tabs();
               }
            }));
         }
      }
      const registerItems = document.querySelectorAll(".spollers-register__item");
      registerItems.forEach((registerItem => {
         const image = registerItem.querySelector("img");
         if (!image) registerItem.querySelector(".spollers-register__title").style.pointerEvents = "none";
      }));
      window["FLS"] = true;
      isWebp();
      menuInit();
